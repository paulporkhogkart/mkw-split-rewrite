...
export class Announcer {
  ...
  constructor(private token: string, private channelId: string) {
    ...
    this._client.once('clientReady', async () => {
      const ch = await this._client.channels.fetch(this.channelId).catch(() => null);
      ...
      this.queue = [];
      console.log(`[bot] logged in as ${this._client.user?.tag}`);
    });
  }

  /** Exposes the discord.js Client so external modules (e.g. installCommands) can attach listeners. */
  get client(): Client { return this._client; }

  async start(): Promise<void> { await this._client.login(this.token); }
