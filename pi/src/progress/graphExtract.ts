...
export function extractGraph(skel: Uint8Array, w: number, h: number): { nodes: RawNode[]; edges: RawEdge[] } {
  ...
  const deg = (i: number, j: number) => { let d = 0; for (const [di, dj] of NB) if (on(i + di, j + dj)) d++; return d; };

  const nodeOf = new Int32Array(w * h).fill(-1);
  const nodes: RawNode[] = [];

  // (a) junction clusters: connected components of degree>=3 pixels collapse to one node (centroid).
  const seen = new Uint8Array(w * h);
  for (let j = 0; j < h; j++) for (let i = 0; i < w; i++) {
    if (!on(i, j) || deg(i, j) < 3 || seen[idx(i, j)]) continue;
    ...
    while (stack.length) {
      ...
      for (const [di, dj] of NB) {
        ...
        if (on(ni, nj) && deg(ni, nj) >= 3 && !seen[idx(ni, nj)]) { seen[idx(ni, nj)] = 1; stack.push([ni, nj]); }
      }
    }
    nodes.push({ id, ci: Math.round(sci / n), cj: Math.round(scj / n) });
  }
  // (b) endpoints (degree 1) -> singleton nodes.
  for (let j = 0; j < h; j++) for (let i = 0; i < w; i++)
    if (on(i, j) && deg(i, j) === 1 && nodeOf[idx(i, j)] === -1) {
  ...
  for (const [pi, pj] of nodePix) {
    ...
    for (const [di, dj] of NB) {
      ...
      while (nodeOf[idx(curI, curJ)] < 0) {                            // walk degree-2 pixels to the next node
        let ni = -1, nj = -1;
