...
import { courseLeaderboardAllTime, currentWr } from './reads';
...
/** Each player's fastest lap-duration per lap index over all-time finished, non-carryover runs,
 *  plus the per-lap field ideal (min across players). `best`/`fieldIdeal` are length `laps`
 *  (max lap seen). Matches the website's all-time timeline/turf model. */
export function courseSplits(db: DatabaseSync, courseId: number, cc: number): CourseSplits {
  ...
  ).all(courseId, cc) as { pid: number; name: string; color: string | null; lap: number; best: number }[];
 ...
 *  All-time (cross-season): leaderboard/splits/history match the /tracks index + turf map's
 *  all-time timeline model, not the active season. */
export function courseSummary(db: DatabaseSync, cc: number, slug: string): CourseSummary | null {
  ...
  const leaderboard: CourseLeaderRow[] = courseLeaderboardAllTime(db, courseId, cc)
  ...
  return {
    ...
    leaderboard,
    splits: courseSplits(db, courseId, cc),
    history: {
      recordProgression: recordProgression(db, courseId, cc),
      reigns: courseReigns(db, courseId, cc),
