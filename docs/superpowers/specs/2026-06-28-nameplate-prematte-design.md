...
5. **Retired vs reused:** the post-matte plate-removal functions `drop_nameplate` + `undark_rgba` (and `tests/test_undark.py`) are **retired** — superseded by pre-matte. `undark.py`'s reusable helpers move to `pre_darken.py`: `load_template(is_char)` (loads the per-screen `t,C,mask`) and `_reencode` (PNG seq → `_loop.webp`/`_checker.webp`/`_apng.png`). `nametag_core.py`, `build_templates.py`, and the committed `assets/` are **kept and reused** unchanged.
