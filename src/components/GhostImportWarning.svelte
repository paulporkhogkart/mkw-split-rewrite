<script>
  ...
  let dialogEl;
  // Move focus onto the dialog so Esc/Tab/Enter act on the modal (mirrors RunReviewModal).
  onMount(() => dialogEl?.focus?.());
</script>

<div class="gi-backdrop" transition:fade={{ duration: 120 }}>
  <div class="gi-dialog" bind:this={dialogEl} tabindex="-1" role="dialog" aria-modal="true" aria-labelledby="gi-title"
...
<style>
  ...
  .gi-dialog { width: 100%; max-width: 360px; display: flex; flex-direction: column;
    ...
    box-shadow: 0 16px 44px rgba(0,0,0,.5); overflow: hidden; outline: none; }
